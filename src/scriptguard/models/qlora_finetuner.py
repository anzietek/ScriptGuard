...
def compute_class_weights(dataset: Dataset, method: str = "sqrt_inverse") -> dict:
    ...
    logger.info("Scanning dataset for class labels using prompts.py anchors...")

    for item in dataset:
        text = item.get("text", "")

        # Check for the actual format generated by prompts.py
        if MALICIOUS_ANCHOR in text:
            labels.append("malicious")
        elif BENIGN_ANCHOR in text:
            labels.append("benign")
        else:
            # Fallback for debugging - check if 'label' column exists directly
            label = item.get("label")
            if label:
                if isinstance(label, int):
                    labels.append("malicious" if label == 1 else "benign")
                else:
    ...
    logger.info(f"Class distribution found: {dict(label_counts)}")

    if "malicious" not in label_counts or "benign" not in label_counts:
        logger.warning("⚠️  WARNING: Could not find both classes in dataset text! Check prompts.py format.")
        # Return neutral weights to prevent crash
...
class WeightedLossTrainer(UnslothTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        for i in range(input_ids.shape[0]):
            try:
                # Decode only the last part to be faster and matching the label
                # The label is always at the end of the prompt
                text = self.processing_class.decode(input_ids[i][-20:], skip_special_tokens=True)
                ...
                if "MALICIOUS" in text:
                    ...
                    # Try decoding full text if short decode failed (fallback)
        ...
        # ... (reszta funkcji bez zmian - aplikacja wag)
