...
class ScriptGuardClassifier:
    ...
    def classify(self, script: str) -> tuple[str, float]:
        """
        Classify a Python script as 'benign' or 'malicious'.

        Returns:
            (label, confidence)  where confidence is malicious probability
            for malicious results, and (1 - malicious_prob) for benign.
        """
        if not script or not script.strip():
            raise InferenceError("Cannot classify empty script")

        # ── Step 1: TrustEngine allowlist ─────────────────────────────────
        if self._trust_engine.is_safe(script):
            logger.info("TrustEngine: safe pattern matched. Returning benign.")
            return "benign", 0.0

        # ── Step 2: Feature extraction ────────────────────────────────────
        features_27d = self._extractor.extract(script)

        # ── Step 3: Heuristic short-circuit ──────────────────────────────
        # Both _gadget_features (39 flags) and _taint_features (30 flags)
        # are included in malware_api_score[21], but we re-evaluate them
        # individually here for the heuristic decision so the threshold
        # can be set independently of the ML model confidence.
        gadget_flags = self._extractor._gadget_features(script)
        taint_flags  = self._extractor._taint_features(
            script,
            None,  # re-parse internally -- acceptable cost for high-value check
            {},
        )
        gadget_hits = int(sum(gadget_flags))
        taint_hits  = int(sum(taint_flags))
        total_hits  = gadget_hits + taint_hits

        if total_hits >= self._heuristic_threshold:
            logger.info(
                f"Heuristic block: {gadget_hits}/39 gadget flags, "
                f"{taint_hits}/30 taint flags. "
                f"Total {total_hits} >= threshold {self._heuristic_threshold}."
            )
            return "malicious", 1.0

        # ── Step 4: Standard AI path ──────────────────────────────────────
        scaled = self._scaler.transform(
            np.array([features_27d], dtype=np.float32)
        )
        feat_tensor = torch.tensor(scaled, dtype=torch.float32).to(self.device)
        ...
        if best_prob >= self._decision_threshold:
            return "malicious", best_prob

        return "benign", 1.0 - best_prob