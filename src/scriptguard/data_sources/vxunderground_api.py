...
Fetches malware samples from VX-Underground GitHub repository.
"""
...
class VXUndergroundDataSource:
    """VX-Underground GitHub integration for malware script samples."""

    GITHUB_API = "https://api.github.com"
    VX_REPO = "vxunderground/MalwareSourceCode"
    ...
    def _make_request(self, url: str) -> Optional[Dict]:
        ...
        def _do_request():
            ...
            if response.status_code == 200:
                ...
                logger.error("GitHub rate limit exceeded (VX-Underground uses GitHub)")
        ...
        try:
            ...
            logger.error(f"VX-Underground request failed after retries: {e}")
            return None
    ...
    def _search_scripts_in_path(self, path: str, extensions: List[str], max_samples: int, depth: int = 0) -> List[Dict]:
        ...
        samples = []
        url = f"{self.GITHUB_API}/repos/{self.VX_REPO}/contents/{path}"
        ...
        for item in data:
            ...
            elif item["type"] == "dir" and len(samples) < max_samples:
    ...
    def fetch_malicious_samples(
        ...
        """
        Fetch malicious script samples from VX-Underground.
        ...
        logger.info(f"Fetching up to {max_samples} samples from VX-Underground")
        ...
        root_url = f"{self.GITHUB_API}/repos/{self.VX_REPO}/contents"
        root_data = self._make_request(root_url)

        if not root_data:
            logger.error("Could not access VX-Underground root")
            return []
        ...
        for priority in priority_paths:
            ...
            for item in found_dirs:
                if priority.lower() in item['name'].lower():
                    logger.info(f"Exploring priority directory: {item['name']}")
        ...
        logger.info(f"Fetched {len(all_samples)} samples from VX-Underground")
