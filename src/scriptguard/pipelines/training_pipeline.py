...
def advanced_training_pipeline(
    ...
    if config.get("augmentation", {}).get("enabled", True):
        ...
        # --- NEW STEP: Post-Augmentation Validation ---
        # Validate augmented samples to ensure obfuscation didn't break syntax.
        # This prevents feeding the model with broken code generated by regex operations.
        logger.info("Validating augmented samples to ensure obfuscation didn't break syntax...")

        augmented_data = _validate_samples(
            data=augmented_data,
            config=config,
            validate_syntax=True,  # Force syntax validation
            min_length=50,
            max_length=100000,
    ...
    logger.info(
        f"Calling balance_dataset step (enabled={config.get('augmentation', {}).get('balance_dataset', True)})...")
    ...
    logger.info(
        f"Calling augment_with_qdrant_patterns step (enabled={config.get('augmentation', {}).get('use_qdrant_patterns', False)})...")
